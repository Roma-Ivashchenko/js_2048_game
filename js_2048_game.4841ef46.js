...
function render() {
    ...
    for(let i = 0; i < grid.length; i++){
        ...
        if (field[i] !== 0) field[i].classList.add('field-cell--' + grid[i]);
    }
...
class Game {
    constructor(initialState){
        this.initialConfig = initialState ? initialState.map((row)=>[
                ...row
            ]) : null;
        if (initialState) this.grid = initialState.map((row)=>[
                ...row
            ]);
        else this.grid = Array(4).fill(null).map(()=>Array(4).fill(0));
        this.score = 0;
        this.status = 'idle';
    }
    processRow(row) {
    ...
    transpose() {
        ...
        for(let r = 0; r < this.grid.length; r++)for(let c = 0; c < this.grid[r].length; c++)result[c][r] = this.grid[r][c];
        this.grid = result;
    }
    canMove() {
        const flat = [
        ...
        if (flat.includes(0)) return true;
    ...
    afterMove(wasChanged) {
        if (this.grid.flat().includes(2048)) this.status = 'win';
        if (wasChanged) this.addRandomTile();
    ...
    processFullGrid(reverseRows = false) {
        ...
        for(let r = 0; r < this.grid.length; r++){
            ...
            const rowToProcess = reverseRows ? [
                ...originRow
            ].reverse() : originRow;
            const row = this.processRow(rowToProcess);
            if (reverseRows) row.reverse();
            if (row.join(',') !== originRow.join(',')) wasChanged = true;
            this.grid[r] = row;
        }
        return wasChanged;
    }
    ...
    addRandomTile() {
        const emptyCells = [];
        for(let r = 0; r < this.grid.length; r++){
            for(let c = 0; c < this.grid[r].length; c++)if (this.grid[r][c] === 0) emptyCells.push({
                r,
                c
            });
        }
    ...
    restart() {
        this.score = 0;
        this.status = 'idle';
        if (this.initialConfig) this.grid = this.initialConfig.map((row)=>[
                ...row
            ]);
        else this.grid = Array(4).fill(null).map(()=>Array(4).fill(0));
    }
